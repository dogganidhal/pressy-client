...
abstract class DataSource {
  ...
  Future<Entity> handleRequest<Entity>(
    ...
    if (responseConverter != null) {
      final map =
          response.contentLength > 0 ? json.decode(response.body) : null;
  ...
  Future<TEntity> doGet<TEntity>(
    ...
    var response =
        await this.client.get(url, headers: this._injectHeaders(headers));
    var map = json.decode(response.body);
    if (response.statusCode >= 400) {
      throw ApiError.fromJson(map);
    }
    return Future.value(
        responseConverter != null ? responseConverter(map) : null);
  }
  ...
  Future<TEntity> doPost<TEntity>(
      ...
      JsonModelBuilder<TEntity> responseConverter}) async {
    var response = await this.client.post(url,
        headers: this._injectHeaders(headers), body: json.encode(body));
    var map = json.decode(response.body);
    if (response.statusCode >= 400) {
      throw ApiError.fromJson(map);
    }
  ...
  Future<TEntity> doPatch<TEntity>(
      ...
      Map<String, dynamic> headers,
      JsonModelBuilder<TEntity> responseConverter}) async {
    var response = await this.client.patch(url,
        headers: this._injectHeaders(headers), body: json.encode(body));
    var map = json.decode(response.body);
    if (response.statusCode >= 400) {
      throw ApiError.fromJson(map);
  ...
  Future<TEntity> doDelete<TEntity>(
      ...
      JsonModelBuilder<TEntity> responseConverter}) async {
    var response =
        await this.client.delete(url, headers: this._injectHeaders(headers));
    var map = json.decode(response.body);
    if (response.statusCode >= 400) {
      throw ApiError.fromJson(map);
    }
  ...
  Map<String, String> _prepareHeaders(ApiEndpoint endpoint) {
    ...
    if (endpoint.needsAuthorization) {
      headers["Authorization"] =
          "Bearer ${this.authSession.credentials?.accessToken}";
    }
    return headers;
  }
  ...
  Map<String, dynamic> _injectHeaders(Map<String, dynamic> headers) {
    var injectableHeaders = {
      "Authorization":
          "${this.authSession.credentials?.tokenType} ${this.authSession.credentials?.accessToken}",
      "Accept": "application/json",
      "Content-Type": "application/json"
    };
    if (headers == null) {
