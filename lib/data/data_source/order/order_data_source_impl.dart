...
class OrderDataSourceImpl extends DataSource implements IOrderDataSource {
  ...
  Future<List<Slot>> getPickupSlots() {
    return this.handleRequest(
        ...
        responseConverter: (json) =>
            json.map((slot) => Slot.fromJson(slot)).toList().cast<Slot>());
  }
  ...
  Future<List<Slot>> getDeliverySlots(Slot pickupSlot) {
    return this.handleRequest(
        endpoint:
            this.apiEndpointProvider.orders.getDeliverySlots(pickupSlot.id),
        responseConverter: (json) =>
            json.map((slot) => Slot.fromJson(slot)).toList().cast<Slot>());
  }
  ...
  Future<List<Order>> getMemberOrders() {
    return this.handleRequest(
        ...
        responseConverter: (json) =>
            json.map((order) => Order.fromJson(order)).toList().cast<Order>());
  }
