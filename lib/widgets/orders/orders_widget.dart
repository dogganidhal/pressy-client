        ...
        bottom: TabBar(
          ...
          tabs: <Widget>[
            ...
            Container(
              padding: EdgeInsets.only(bottom: 8, top: 8),
              child:
                  Text("À VENIR", style: TextStyle(color: ColorPalette.orange)),
            )
          ],
        ),
      ),
      ...
      return state.pastOrders.length > 0
          ? ListView.builder(
              ...
              itemBuilder: (context, index) =>
                  this._orderWidget(state.pastOrders[index]),
    ...
    if (state is OrdersLoadingState) {
      ...
      return state.ongoingOrders.length > 0
          ? ListView.builder(
              ...
              itemBuilder: (context, index) =>
                  this._orderWidget(state.ongoingOrders[index]),
              padding: EdgeInsets.all(8),
            )
    ...
    if (state is OrdersLoadingState) {
      ...
      return state.futureOrders.length > 0
          ? ListView.builder(
              ...
              itemBuilder: (context, index) =>
                  this._orderWidget(state.futureOrders[index]),
              padding: EdgeInsets.all(8),
            )
